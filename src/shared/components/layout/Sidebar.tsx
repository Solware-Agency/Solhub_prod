    ...
    >
      ...
      <div
        className={cn(
          ...
          isExpanded
            ? 'max-h-96 transition-[max-height] duration-600'
            : 'max-h-0 transition-[max-height] duration-400',
        )}
      >
        {children}
      </div>
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  React.useEffect(() => {
    console.log('🔴 isHelpModalOpen changed to:', isHelpModalOpen);
  }, [isHelpModalOpen]);
  ...
  return (
    <aside className='bg-white/80 dark:bg-background/50 shadow-lg shadow-primary/50 backdrop-blur-[3px] dark:backdrop-blur-[10px] flex flex-col h-screen py-4 sm:py-6 px-2 sm:px-4 gap-0 text-gray-700 dark:text-white ease-in-out overflow-hidden border-r border-input'>
          ...
          <FeatureGuard feature='hasChatbot'>
            <div
              onClick={(e) => {
                ...
                e.preventDefault();
                e.stopPropagation();
                console.log('🔵 Setting isHelpModalOpen to true');
      ...
      {/* Modal de Ayuda */}
      {console.log('🔴 Rendering HelpChatbotModal with isOpen:', isHelpModalOpen)}
      <HelpChatbotModal 
