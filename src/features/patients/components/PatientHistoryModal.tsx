...
import {
	getDependentsByResponsable,
	getResponsableByDependiente,
	deleteResponsibility,
} from '@/services/supabase/patients/responsabilidades-service'
...
const PatientHistoryModal: React.FC<PatientHistoryModalProps> = ({
	...
	const [representadoToDelete, setRepresentadoToDelete] = useState<{
	...
	const { isGeneratingPDF, isSaving, handleCheckAndDownloadPDF, getPDFBlob } = usePDFDownload()
	...
	const toggleSelectAllRepresentado = () => {
		...
		const approvedCases = filteredDependentsCases.filter((caseItem) => caseItem.doc_aprobado === 'aprobado')
		if (selectedRepresentadoCases.size === approvedCases.length) {
			setSelectedRepresentadoCases(new Set())
		} else {
			setSelectedRepresentadoCases(new Set(approvedCases.map((caseItem) => caseItem.id)))
		}
	}
	...
	const handleDownloadMultiplePDFsRepresentado = async () => {
		...
		const casesToDownload =
			filteredDependentsCases?.filter(
				(caseItem) => selectedRepresentadoCases.has(caseItem.id) && caseItem.doc_aprobado === 'aprobado',
			) || []
		if (casesToDownload.length === 0) {
			toast({
				title: '⚠️ No hay casos válidos',
				description: 'Los casos seleccionados deben estar aprobados para poder descargar sus PDFs.',
				variant: 'destructive',
			})
			return
		}
		...
		try {
			...
			for (let i = 0; i < casesToDownload.length; i++) {
				...
				try {
					...
					errorCount++
					errors.push(
						`Error: ${caseItem.code || caseItem.id}: ${error instanceof Error ? error.message : 'Error desconocido'}`,
					)
				}
			}
			if (successCount === 0) {
			...
			const representadoName =
				representadosList
					?.find((r) => r.id === selectedRepresentadoId)
	...
	const isRepresentado = patient
		? (patient as any).tipo_paciente === 'menor' || (patient as any).tipo_paciente === 'animal'
		: false
	...
	const {
		data: dependentsCases,
		isLoading: isLoadingDependentsCases,
		refetch: refetchDependentsCases,
	} = useQuery({
	...
	const toggleSelectAll = () => {
		...
		const approvedCases = filteredCases.filter((caseItem) => caseItem.doc_aprobado === 'aprobado')
		...
		if (selectedCases.size === approvedCases.length) {
			...
			const allApprovedIds = new Set(approvedCases.map((caseItem) => caseItem.id))
	...
	const handleDownloadMultiplePDFs = async () => {
		...
		const casesToDownload =
			filteredCases?.filter((caseItem) => selectedCases.has(caseItem.id) && caseItem.doc_aprobado === 'aprobado') || []
	...
	const handleConfirmSendMultipleEmails = async (emails: { to: string; cc: string[]; bcc: string[] }) => {
		...
		try {
			...
			toast({
				title: '❌ Error',
				description: 'Hubo un problema al enviar los emails. Por favor intenta de nuevo.',
				variant: 'destructive',
			})
		} finally {
	...
	const handleSendAllEmails = () => {
		...
		const approvedCases =
			filteredCases?.filter((c) => {
				...
				const hasImages =
					((c as any)?.images_urls && Array.isArray((c as any).images_urls) && (c as any).images_urls.length > 0) ||
					!!(c as any)?.image_url
		...
		<AnimatePresence>
													...
													<p className="text-sm text-gray-600 dark:text-gray-400">
														{isRepresentado && responsableData?.responsable ? (
															<>
																...
																{responsableData.responsable.cedula || 'Sin cédula'}
																...
																{(() => {
																	...
																	if (patient.fecha_nacimiento && calculatedAge) {
																		try {
																			...
																			return <span className="ml-3">• {calculatedAge}</span>
																		}
																	}

																	// Si no hay fecha_nacimiento pero hay edad directa, mostrarla
																	if (patient.edad) {
																		return <span className="ml-3">• {patient.edad}</span>
																	}
																	...
																	if (patient.fecha_nacimiento) {
																		try {
																			...
																			if (!isNaN(fechaNac.getTime())) {
																				const fechaFormateada = format(fechaNac, 'dd/MM/yyyy', { locale: es })
																				return <span className="ml-3">• ({fechaFormateada})</span>
																			}
																		} catch (error) {
																			// Ignorar error
																			console.error('Error formateando fecha de nacimiento:', error)
									...
									<div className="flex-1 overflow-hidden flex flex-col min-h-0 h-full">
										...
										>
											<div className="p-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
												<TabsList
													className={`grid w-full gap-5 ${laboratory?.features?.hasTriaje ? 'grid-cols-3' : 'grid-cols-2'}`}
												>
													...
													<TabsTrigger value="representados" className="flex items-center gap-2">
											...
											<TabsContent value="cases" className="mt-0 flex-1 overflow-hidden flex flex-col min-h-0">
												{/* Search and Filters */}
												<div className="p-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
															...
															{filteredCases.map((caseItem: MedicalCaseWithPatient, index: number) => (
																<div
																	...
																	className={`bg-white/60 dark:bg-background/30 backdrop-blur-[5px] border rounded-lg p-4 transition-all duration-200 cursor-pointer hover:shadow-lg hover:shadow-primary/10 dark:hover:shadow-primary/20 hover:border-primary/50 hover:bg-white/80 dark:hover:bg-background/50 hover:scale-[1.01] ${
																		selectedCases.has(caseItem.id) ? 'border-primary border-2' : 'border-input'
																	}`}
																>
																		...
																		<div className="grid grid-cols-3 gap-3">
																			<div>
																				...
																				{showCodeBadge(caseItem) ? (
																					...
																					<p className="text-sm font-medium text-gray-400">Sin código</p>
																				)}
																			</div>
																			<div>
																				...
																				<p className="text-sm font-medium flex items-center gap-1">
																					...
																					{format(new Date(caseItem.created_at || caseItem.date), 'dd/MM/yyyy', {
																						locale: es,
																					})}
																				</p>
																			</div>
											...
											{/* Tab: Representados */}
											<TabsContent value="representados" className="mt-0 flex-1 overflow-hidden flex flex-col min-h-0">
												{/* Two Panel Layout */}
												<div className="flex-1 flex overflow-hidden min-h-0">
													...
													<div className="w-1/3 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
														<div className="p-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
															<h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Representados</h3>
														</div>
													...
													<div className="flex-1 flex flex-col overflow-hidden">
														...
														<div className="flex-1 overflow-y-auto p-4">
															{!selectedRepresentadoId ? (
																...
																<div className="text-center py-12">
																	<p className="text-red-500 dark:text-red-400">Error al cargar casos</p>
																</div>
																					...
																					<div className="flex items-center gap-2">
																						{caseItem.doc_aprobado === 'aprobado' && (
																							<Checkbox
																								...
																								onCheckedChange={() => toggleRepresentadoCaseSelection(caseItem.id)}
																								disabled={isDownloadingMultiple || isGeneratingPDF || isSaving}
																								className="mr-1"
																							/>
																						)}
											...
											<TabsContent value="triage" className="mt-0 flex-1 overflow-hidden flex flex-col min-h-0">
												<div className="flex-1 overflow-y-auto p-4 min-h-0 h-full">
													<TriageHistoryTab patientId={patient?.id || ''} isOpen={isOpen} />
												</div>
											</TabsContent>
										</Tabs>
									</div>
			...
			{patient && (
				<SendEmailModal
					...
					patientName={patient.nombre || ''}
					caseCode={
						pendingEmailCases.length > 1 ? `${pendingEmailCases.length} casos` : pendingEmailCases[0]?.code || ''
					}
							...
							onClick={async () => {
								...
								try {
									...
									if (selectedRepresentadoId === representadoToDelete.dependienteId) {
										const remaining =
											representadosList?.filter((r) => r.id !== representadoToDelete.dependienteId) ?? []
