...
interface CaseDetailPanelProps {
	...
	isOpen: boolean
	onClose: () => void
	/** Se llama cuando termina la animación de cierre; el padre puede limpiar el caso aquí para no desmontar antes del exit */
...
function CaseModalPortalContent({
	isOpen,
	overlayContent,
	rest,
}: {
...
const UnifiedCaseModal: React.FC<CaseDetailPanelProps> = React.memo(
	({ case_, isOpen, onClose, onCloseComplete, onSave, onDelete, isFullscreen = false, modalTitle = 'Detalles Del Caso' }) => {
		...
		const examTypesOptions = useMemo(() => {
			...
			if (examTypes.length > 0) {
				...
				const sortedExamTypes = [...examTypes].sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }))
				return createDropdownOptions(sortedExamTypes.map((type) => ({ value: type, label: type })))
			}
		...
		const branchOptions = useMemo(() => {
			...
			if (branches.length > 0) {
				return createDropdownOptions(branches.map((branch) => ({ value: branch, label: branch })))
			}
		...
		const paymentMethodsOptions = useMemo(() => {
			...
			if (paymentMethods.length > 0) {
				return createDropdownOptions(paymentMethods.map((method) => ({ value: method, label: method })))
			}
			// Fallback a valores por defecto si no hay configuración
			return createDropdownOptions([
    ...
    */
		...
		const handleSaveChanges = async () => {
			...
			try {
				...
				for (const field of caseFields) {
					...
					const hasChange =
						field === 'owner_display_code'
							? (() => {
									const oldNorm = String((currentCase as any).owner_display_code ?? '')
										.replace(/\D/g, '')
										.slice(0, 5)
									const newNorm = String(editedCase.owner_display_code ?? '')
										.replace(/\D/g, '')
										.slice(0, 5)
		...
		// No retornar null por !currentCase al cerrar: el padre debe mantener case_ hasta onCloseComplete para que AnimatePresence ejecute el exit
		if (!case_) return null
		// Tipado seguro en el render cuando case_ existe
		const caseData = currentCase ?? case_!
		...
		const currentPaymentMethods: PaymentMethod[] = []
		for (let i = 1; i <= 4; i++) {
			const method = caseData[`payment_method_${i}` as keyof MedicalCaseWithPatient] as string
			const amount = caseData[`payment_amount_${i}` as keyof MedicalCaseWithPatient] as number
			const reference = caseData[`payment_reference_${i}` as keyof MedicalCaseWithPatient] as string
											...
											{isEditing ? (
												...
												<>
													...
													{/* <button
														...
														className="inline-flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-2 text-xs font-semibold rounded-md bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800/40 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
		...
		// Portal con componente estable para que AnimatePresence no se desmonte y pueda ejecutar exit
		if (isFullscreen) {
			return ReactDOM.createPortal(
				<CaseModalPortalContent
					isOpen={isOpen}
					overlayContent={isOpen ? overlayContentJSX : null}
					rest={restContent}
				/>,
				document.body,
			)
