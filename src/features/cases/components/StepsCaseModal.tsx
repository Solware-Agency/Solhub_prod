...
const StepsCaseModal: React.FC<StepsCaseModalProps> = ({ case_, isOpen, onClose, onSuccess, isFullscreen = false }) => {
	...
	const computedSteps = useMemo(() => {
		...
		if ((isOwner || isCitotecno || isMedicowner) && isCitology && laboratory?.features?.hasEvaluateCitology) {
			stepsList.push({
				id: 'citology',
				title: 'Citología',
				icon: FileCheck,
		...
		const shouldShowApproveStep = isSptAutoApprove ? false : (isCitotecno && isCitology) || isOwner || isMedicowner
	...
	const getInitialStep = () => {
		...
		if (
			...
			const citologyStepIndex = computedSteps.findIndex((step) => step.id === 'citology')
			// Si el paso de citología existe, usarlo; si no, ir al paso de aprobación
			return citologyStepIndex !== -1 ? citologyStepIndex : computedSteps.findIndex((step) => step.id === 'approve')
		}

		if (!isSpt && isOwner && docAprobado === 'pendiente' && isCitology && citoStatus === 'positivo') {
			return computedSteps.findIndex((step) => step.id === 'approve')
		}
	...
	const handleConfirmSendEmail = async (emails: { to: string; cc: string[]; bcc: string[] }) => {
		...
		try {
			...
			const caseImages =
				(case_ as any)?.images_urls &&
				Array.isArray((case_ as any).images_urls) &&
				(case_ as any).images_urls.length > 0
					? (case_ as any).images_urls
					: (case_ as any)?.image_url
						? [(case_ as any).image_url]
						: []
				...
				return (
					...
					>
						<div className="grid gap-4">
							...
							<div className="bg-linear-to-br from-teal-50 to-cyan-50 dark:from-teal-900/20 dark:to-cyan-900/20 p-4 rounded-lg border border-teal-200 dark:border-teal-800">
								<div className="flex flex-col sm:flex-row sm:items-end gap-2 sm:gap-3">
									{isMarihorgen && (
										<div className="w-full sm:max-w-xs">
											<label className="text-sm font-medium text-foreground">Plantilla de informe</label>
											<Select
							...
							<div className="bg-linear-to-br from-teal-50 to-cyan-50 dark:from-teal-900/20 dark:to-cyan-900/20 p-4 rounded-lg border border-teal-200 dark:border-teal-800">
								{isNotRechazado ? (
									<p className="text-teal-400 text-sm">Rellena los datos y regresa a esta pestaña para continuar.</p>
								) : (
								...
								<div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
									<Button
										...
										disabled={
											...
											(isSptAutoApprove ? false : docAprobado != 'faltante' && docAprobado !== 'rechazado') ||
											!docUrl
										}
									>
								...
								<div className="bg-background px-6 py-4">
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-3">
											<Sparkles className="w-6 h-6 text-black dark:text-white shrink-0" />
										...
										<button onClick={handleClose} className="p-1 hover:bg-white/20 rounded-lg transition-none shrink-0">
														...
														<div className="mt-2 text-center">
															<p
																className={`text-xs font-medium ${
																	isActive ? 'text-labPrimary' : 'text-gray-600 dark:text-gray-400'
																}`}
															>
																{step.title}
															</p>
