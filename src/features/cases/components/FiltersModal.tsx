...
const FiltersModal: React.FC<FiltersModalProps> = ({
	...
	const hasActiveFilters =
		...
		(usesFechaMuestra && (sampleDateRange?.from != null || sampleDateRange?.to != null)) ||
		(usesFechaMuestra && sampleTypeFilter != null && sampleTypeFilter !== '' && sampleTypeFilter !== 'all')
	...
	return (
		<Dialog open={isOpen} onOpenChange={onOpenChange}>
					...
					{hasActiveFilters && (
						<span className="bg-white dark:bg-gray-800 text-primary text-xs px-2 py-0.5 rounded-full">
							{[
								...
								usesFechaMuestra && (sampleDateRange?.from != null || sampleDateRange?.to != null) ? 1 : 0,
								usesFechaMuestra && sampleTypeFilter != null && sampleTypeFilter !== '' && sampleTypeFilter !== 'all'
									? 1
									: 0,
							].reduce((a, b) => a + b, 0)}
			...
			>
				...
				<DialogDescription className="sr-only">Configure los filtros para buscar casos específicos</DialogDescription>
					...
					<TabsContent value="general" className="space-y-6 mt-6 overflow-x-hidden overflow-y-auto flex-1 pb-4">
						...
						{usesFechaMuestra && (onSampleDateRangeChange || onSampleTypeFilterChange) && (
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
								{onSampleDateRangeChange && (
									<div className="space-y-3">
										<DatePopover open={isSampleDateRangeOpen} onOpenChange={setIsSampleDateRangeOpen} modal={false}>
											<DatePopoverTrigger asChild>
												<Button variant="outline" className="flex items-center gap-2 w-full justify-start font-bold">
													<CalendarIcon className="w-4 h-4" />
								...
								<div className="flex flex-wrap gap-2">
									...
									{emailSentStatusFilter !== 'all' && (
										<span className="inline-flex items-center gap-1 px-3 py-1 bg-teal-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 text-sm rounded-full">
											Envío: {emailSentStatusFilter === 'true' ? 'Enviado' : 'No Enviado'}
											<button
