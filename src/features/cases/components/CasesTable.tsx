...
import { ChevronUp, ChevronDown, Search, Download, X } from 'lucide-react'
...
type SortField = 'id' | 'created_at' | 'fecha_entrega' | 'nombre' | 'total_amount' | 'code'
...
const CasesTable: React.FC<CasesTableProps> = React.memo(
	({
		...
		const [sampleDateRange, setSampleDateRange] = useState<DateRange | undefined>(undefined)
		...
		const [emailSentStatusFilter, setEmailSentStatusFilter] = useState<string>('all')
		...
		const [selectedCaseForTriaje, setSelectedCaseForTriaje] = useState<UnifiedMedicalRecord | null>(null)
		...
		const [tempEmailSentStatusFilter, setTempEmailSentStatusFilter] = useState<string>('all')
		...
		const [tempSampleDateRange, setTempSampleDateRange] = useState<DateRange | undefined>(undefined)
			...
			(field: SortField) => {
				...
				if (pagination && onFiltersChange) {
					...
					if (hasTriaje && triageStatusFilter !== 'all') {
						currentFilters.triageStatus = triageStatusFilter as 'pendiente' | 'completo'
					}
					if (sampleDateRange?.from || sampleDateRange?.to) {
						...
						const fromStr = sampleDateRange.from ? formatYmd(sampleDateRange.from) : null
						const toStr = sampleDateRange.to ? formatYmd(sampleDateRange.to) : null
		...
		const handleApplyFilters = useCallback(() => {
			...
			if (pagination && onFiltersChange) {
				...
				if (tempSampleDateRange?.from || tempSampleDateRange?.to) {
					...
					const fromStr = tempSampleDateRange.from ? formatYmd(tempSampleDateRange.from) : null
					const toStr = tempSampleDateRange.to ? formatYmd(tempSampleDateRange.to) : null
		...
		const handleTempSampleDateRangeChange = useCallback((range: DateRange | undefined) => {
		...
		const handleTempEmailSentStatusFilterChange = useCallback((value: string) => {
			setTempEmailSentStatusFilter(value)
		}, [])
		const handleTempTriageStatusFilterChange = useCallback((value: string) => {
		...
		const filteredAndSortedCases = useMemo(() => {
			...
			const filtered = casesToProcess.filter((case_: UnifiedMedicalRecord) => {
				...
				const matchesBranch =
					branchFilter.length === 0 || branchFilter.some((branch) => normalize(case_.branch) === normalize(branch))
							...
							<div className="flex flex-col md:flex-row md:items-center gap-2 sm:gap-4">
								...
								<div className="flex flex-wrap items-center gap-2 sm:gap-3 md:shrink-0">
									...
									<FiltersModal
										...
										onEmailSentStatusFilterChange={handleTempEmailSentStatusFilterChange}
								...
								<div className="bg-gray-50/50 dark:bg-background/50 backdrop-blur-[10px] border-b border-gray-200 dark:border-gray-700 px-3 sm:px-4 md:px-6 py-3 shrink-0">
									<div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4">
										...
										<Badge variant="secondary" className="text-xs sm:text-sm font-medium" aria-live="polite">
											{pagination?.totalItems ?? filteredAndSortedCases.filtered.length} caso
											{(pagination?.totalItems ?? filteredAndSortedCases.filtered.length) !== 1 ? 's' : ''}
										</Badge>
									</div>
								</div>
