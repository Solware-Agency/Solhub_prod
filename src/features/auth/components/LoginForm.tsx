...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status) =>
        console.log('[RT][Login] channel status:', status),
      );
  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (signInError) {
        ...
        if (signInError.message.includes('Invalid login credentials')) {
          ...
          retryTimerRef.current = window.setInterval(async () => {
            const { user: retryUser, error: retryErr } = await signIn(
              email,
              password,
            );
          ...
          console.log(
            'Email not confirmed, redirecting to verification notice',
          );
      ...
      setError(
        'Error al iniciar sesión. Verifica tus credenciales o crea una cuenta.',
      );
    } finally {
