    ...
    const checkSession = async () => {
      try {
        ...
        if (recoveryMode && recoveryCode) {
          console.log(
            'No session found, but recovery code is available. Attempting to exchange...',
          );
          try {
            ...
            if (exchangeError) {
              console.error(
                'Error exchanging recovery code for session:',
                exchangeError,
              );
              // Check if it's a PKCE/grant_type error
              if (
                exchangeError.message.includes('grant_type') ||
                ...
                setError(
                  'Error al procesar el código de recuperación. Por favor, solicita un nuevo enlace.',
                );
              }
              setSessionStatus('invalid');
              return;
            ...
            setError(
              'No se pudo establecer una sesión. Por favor, solicita un nuevo enlace.',
            );
            setSessionStatus('invalid');
          } catch (err) {
            console.error('Unexpected error exchanging code:', err);
            setError(
              'Error inesperado. Por favor, solicita un nuevo enlace de restablecimiento.',
            );
            setSessionStatus('invalid');
          }
          return;
        ...
        setError(
          'No hay una sesión activa. Por favor, solicita un nuevo enlace de restablecimiento.',
        );
        setSessionStatus('invalid');
      } catch (err) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Auth session missing')) {
          setError(
            'La sesión ha expirado. Por favor, solicita un nuevo enlace de restablecimiento.',
          );
          setSessionStatus('invalid');
          return;
        }
