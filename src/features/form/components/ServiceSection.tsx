...
const sampleTypeOptionsFromCosts = (costs: SampleTypeCost[]) =>
	costs.map((c) => ({ value: c.name, label: c.name })).sort((a, b) => a.label.localeCompare(b.label, 'es'))

export const ServiceSection = memo(({ control, inputStyles, sampleTypeCosts }: ServiceSectionProps) => {
	...
	const numberOfSamplesConfig = useModuleField('registrationForm', 'numberOfSamples')
	const relationshipConfig = useModuleField('registrationForm', 'relationship')
	...
	const consultaOptions = useMemo(() => {
		...
		const sortedOptions = [...options].sort((a, b) => a.label.localeCompare(b.label, 'es', { sensitivity: 'base' }))
	...
	const formatTitleCase = (value: string): string =>
		value.toLowerCase().replace(/(^|[\s"'(])([a-záéíóúñü])/g, (_, prefix, char) => `${prefix}${char.toUpperCase()}`)
				...
				<FormField
					...
					render={({ field, fieldState }) => (
						<FormItem className="min-w-45 flex-1">
							...
							<FormControl>
								<FormDropdown
									options={createDropdownOptions(!profile?.assigned_branch ? branchOptions : [profile.assigned_branch])}
									value={field.value}
									onChange={field.onChange}
