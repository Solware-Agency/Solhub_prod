-- =====================================================
-- search_patients_optimized: evitar statement_timeout
-- =====================================================
-- 1) Término solo dígitos: solo igualdad en cédula/teléfono (índices).
--    No ejecutar rama FTS+ILIKE (causaba timeouts con muchos pacientes).
-- 2) Término texto: FTS e ILIKE en columnas separadas con UNION ALL + LIMIT
--    cada rama (mejor plan que un OR gigante).
-- 3) ILIKE parcial en cédula/teléfono solo si length(term) >= 4 (menos ruido).
-- =====================================================
...
DECLARE
  term_trimmed TEXT;
  fts_limit INTEGER;
  term_is_numeric BOOLEAN;
  min_ilike_len INTEGER := 4;
BEGIN
  term_trimmed := TRIM(search_term);
  fts_limit := LEAST(GREATEST(result_limit * 3, 25), 75);
  term_is_numeric := (term_trimmed ~ '^[0-9]+$');

  IF term_trimmed = '' THEN
    RETURN;
  END IF;

  -- Solo dígitos: búsqueda indexada por igualdad, sin FTS ni ILIKE.
  IF term_is_numeric THEN
    RETURN QUERY
    SELECT
      p.id,
      p.nombre::TEXT,
    ...
    WHERE p.laboratory_id = search_patients_optimized.lab_id
      ...
      AND (
        p.cedula = term_trimmed
        OR p.telefono = term_trimmed
      )
  ...
  fts_only AS (
    ...
    WHERE p.laboratory_id = search_patients_optimized.lab_id
      AND p.is_active = true
      AND to_tsvector('solhub_unaccent', COALESCE(p.nombre, '')) @@ plainto_tsquery('solhub_unaccent', term_trimmed)
  ...
  ilike_cedula AS (
    SELECT
      ...
      p.fecha_nacimiento AS fts_fecha_nacimiento,
      p.especie::TEXT AS fts_especie,
      0.4::REAL AS fts_score
    FROM patients p
    WHERE p.laboratory_id = search_patients_optimized.lab_id
      AND p.is_active = true
      AND char_length(term_trimmed) >= min_ilike_len
      AND COALESCE(p.cedula, '') ILIKE '%' || term_trimmed || '%'
      AND NOT EXISTS (SELECT 1 FROM exact_matches em WHERE em.exact_id = p.id)
      AND NOT EXISTS (SELECT 1 FROM fts_only fo WHERE fo.fts_id = p.id)
    LIMIT fts_limit
  ),
  ilike_telefono AS (
    SELECT
      ...
      p.fecha_nacimiento AS fts_fecha_nacimiento,
      p.especie::TEXT AS fts_especie,
      0.35::REAL AS fts_score
    FROM patients p
    WHERE p.laboratory_id = search_patients_optimized.lab_id
      AND p.is_active = true
      AND char_length(term_trimmed) >= min_ilike_len
      AND COALESCE(p.telefono, '') ILIKE '%' || term_trimmed || '%'
      AND NOT EXISTS (SELECT 1 FROM exact_matches em WHERE em.exact_id = p.id)
      AND NOT EXISTS (SELECT 1 FROM fts_only fo WHERE fo.fts_id = p.id)
      AND NOT EXISTS (SELECT 1 FROM ilike_cedula ic WHERE ic.fts_id = p.id)
    LIMIT fts_limit
  ),
  fts_matches AS (
    SELECT * FROM fts_only
    UNION ALL
    SELECT * FROM ilike_cedula
    UNION ALL
    SELECT * FROM ilike_telefono
  ),
  combined AS (
...
COMMENT ON FUNCTION public.search_patients_optimized IS
  'Búsqueda de pacientes: término numérico solo igualdad (sin FTS/ILIKE). Texto: FTS + ILIKE cédula/tel en ramas UNION con LIMIT para evitar timeouts.';
