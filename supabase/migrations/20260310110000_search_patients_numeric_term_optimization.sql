-- =====================================================
-- Búsqueda por término numérico (ej. 17642) sin full scan
-- =====================================================
-- Si el término es solo dígitos, en exact_matches NO evaluamos
-- unaccent(nombre), así el plan puede usar índices en cedula/telefono
-- y evitar escanear todas las filas del lab.
-- =====================================================
...
BEGIN
  ...
  WITH exact_matches AS (
    ...
    WHERE p.laboratory_id = search_patients_optimized.lab_id
      ...
      AND (
        (term_is_numeric AND (p.cedula = search_patients_optimized.search_term OR p.telefono = search_patients_optimized.search_term))
        OR
        (NOT term_is_numeric AND (
...
COMMENT ON FUNCTION public.search_patients_optimized IS
  'Búsqueda de pacientes. Término solo numérico usa índices cedula/telefono en exact_matches para evitar timeout.';
