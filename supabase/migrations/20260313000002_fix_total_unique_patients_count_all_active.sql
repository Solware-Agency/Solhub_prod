-- Corregir totalUnique en get_patients_count_by_branch para contar TODOS los pacientes activos
-- (no solo los que tienen casos con branch asignado), igual que en estadísticas.
...
BEGIN
  ...
  -- Total de pacientes únicos: TODOS los pacientes activos (igual que en estadísticas)
  -- No filtrar por casos con branch, solo contar pacientes activos del laboratorio
  SELECT count(*)::int
  INTO v_total_unique
  FROM patients p
...
COMMENT ON FUNCTION public.get_patients_count_by_branch() IS
  'Retorna conteo de pacientes activos por sede para el laboratorio del usuario. totalUnique cuenta TODOS los pacientes activos (igual que estadísticas).';
