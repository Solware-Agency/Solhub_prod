...
function getReminderSubject(type: ReminderType): string {
	switch (type) {
		...
		case 'deactivated':
			return 'Su servicio SolHub ha sido desactivado por falta de pago'
		default:
...
function getReminderMessage(
	...
	if (type === 'deactivated') {
		return `Le informamos que el servicio de **${labName}** en SolHub ha sido **desactivado** por no haber regularizado el pago dentro del plazo de 24 horas tras la fecha de vencimiento (${nextPaymentDate}).\n\nNadie de su organización puede usar el sistema hasta que se reactive el servicio. Para reactivar, contacte a soporte o realice el pago correspondiente.`
	}
	const amountStr = formatAmountLine(billingAmount, exchangeRate, isBcvRate)
	switch (type) {
		case '15_days':
			return `Le recordamos que la próxima fecha de pago de **${labName}** en SolHub es el **${nextPaymentDate}** (en 15 días).${amountStr}\n\nPor favor, realice el pago a tiempo para mantener su servicio activo.`
		case '7_days':
			return `Le recordamos que la próxima fecha de pago de **${labName}** en SolHub es el **${nextPaymentDate}** (en 7 días).${amountStr}\n\nPor favor, realice el pago a tiempo para mantener su servicio activo.`
		case '1_day':
			return `Le recordamos que la próxima fecha de pago de **${labName}** en SolHub es **mañana** (${nextPaymentDate}).${amountStr}\n\nPor favor, realice el pago para evitar interrupciones.`
		case 'due_today':
			return `**Importante:** La fecha de pago de **${labName}** en SolHub es **hoy** (${nextPaymentDate}).${amountStr}\n\nTiene hasta 24 horas para regularizar el pago; después de ese plazo su servicio quedará inactivo.`
		default:
			return `Recordatorio de pago para ${labName}. Próxima fecha: ${nextPaymentDate}.${amountStr}`
	}
}

function buildReminderHtml(ownerName: string, subject: string, messagePlain: string, labName: string): string {
