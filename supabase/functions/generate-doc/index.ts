// Edge Function: generate-doc — proxy a n8n con JWT
// Reemplaza llamada directa desde frontend a VITE_GENERATE_DOC_WEBHOOK
...
Deno.serve(async (req: Request) => {
  ...
  const defaultWebhookUrl = Deno.env.get("GENERATE_DOC_WEBHOOK_URL");
  ...
  if (laboratoryId) {
    ...
    if (serviceKey) {
      ...
      const { data: lab } = await sbAdmin
        ...
        .eq("id", laboratoryId)
        .single();
      const labRow = lab as { config?: { webhooks?: { generateDoc?: string } } } | null;
      const labWebhook = labRow?.config?.webhooks?.generateDoc;
  ...
  if (!webhookUrlToUse) {
    return new Response(
      JSON.stringify({
        success: false,
        error: "URL del webhook de generación de documento no configurada. Configura GENERATE_DOC_WEBHOOK_URL o laboratories.config.webhooks.generateDoc para este laboratorio.",
      }),
